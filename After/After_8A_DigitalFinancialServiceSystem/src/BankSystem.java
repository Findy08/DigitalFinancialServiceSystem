...
public class BankSystem extends JFrame implements ActionListener, ItemListener {
	...
	public BankSystem () {
		...
		super ("Digital Financial Service System");
		...
		end = new JMenuItem ("Quit Digital Financial Service System ?", new ImageIcon (ClassLoader.getSystemResource("Images/export.gif")));
		...
		about = new JMenuItem ("About Digital Financial Service System", new ImageIcon (ClassLoader.getSystemResource("Images/Save.gif")));
		...
		report = new JMenuItem ("Print Digital Financial Service System Report", new ImageIcon (ClassLoader.getSystemResource("Images/New.gif")));
		...
		String[] str = {
				"Images/NotePad.gif", "Create New Account",
				"Images/ImationDisk.gif","Deposit Money",
				"Images/SuperDisk.gif","Withdraw Money",
				"Images/Paproll.gif","Print Customer Balance",
				"Images/Toaster.gif","Delete Customer",
				"Images/Search.gif","Search Customer",
				"Images/Help.gif","Help on Digital Financial Service System",
				"Images/Keys.gif","Shortcut Keys of Digital Financial Service System"
				};
		int i=0;
		btnNew  = createToolbarButton(str[i*2],str[i*2+1]); i++;
		btnDep  = createToolbarButton(str[i*2],str[i*2+1]); i++;
		btnWith = createToolbarButton(str[i*2],str[i*2+1]); i++;
		btnRec = createToolbarButton(str[i*2],str[i*2+1]); i++;
		btnDel = createToolbarButton(str[i*2],str[i*2+1]); i++;
		btnSrch = createToolbarButton(str[i*2],str[i*2+1]); i++;
		btnHelp = createToolbarButton(str[i*2],str[i*2+1]); i++;
		btnKey = createToolbarButton(str[i*2],str[i*2+1]); i++;
		...
		author = new JLabel (" " + "Digital Financial Service System", Label.LEFT);
	...
	private JButton createToolbarButton(String image, String tooltip) {
		JButton b = new JButton (new ImageIcon (ClassLoader.getSystemResource(image)));
	...
	public void actionPerformed (ActionEvent ae) {
		...
		if (obj == addNew || obj == open || obj == btnNew) {
			...
			boolean b = openChildWindow ("Digital Financial Service System Help");
			if (b == false) {
				BankHelp hlpBank = new BankHelp ("Digital Financial Service System Help", "Help/Bank.htm");
				desktop.add (hlpBank);
				hlpBank.show ();
			}
			...
			boolean b = openChildWindow ("Digital Financial Service System Keys");
			if (b == false) {
				BankHelp hlpKey = new BankHelp ("Digital Financial Service System Keys", "Help/Keys.htm");
				desktop.add (hlpKey);
				hlpKey.show ();
			}
			...
			String msg = "Digital Financial Service System\n\n" + "Created & Designed By:\n" + 
				"Muhammad Wasif Javed\n\n" + "E-mail me:\n wasi_javed@hotmail.com";
			JOptionPane.showMessageDialog (this, msg, "About Digital Financial Service System", JOptionPane.PLAIN_MESSAGE);
		...
		try {
			//Show a Confirmation Dialog.
		    	int reply = JOptionPane.showConfirmDialog (this,
					"Are you really want to exit\nFrom Digital Financial Service System?",
					"Digital Financial Service System - Exit", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE);
			//Check the User Selection.
			if (reply == JOptionPane.YES_OPTION) {
				setVisible (false);	//Hide the Frame.
				dispose();            	//Free the System Resources.
				System.out.println ("Thanks for Using Digital Financial Service System\nAuthor - Muhammad Wasif Javed");
	...
	void getAccountNo () {
		...
		if (b == false) { }
			...
			try {
				...
				"(Tip: Account No. Contains only Digits)", "Digital Financial Service System - PrintRecord", JOptionPane.PLAIN_MESSAGE);
				if (printing == null) { }
				if (printing.equals ("")) {
					JOptionPane.showMessageDialog (this, "Provide Account No. to Print.",
						 "Digital Financial Service System - EmptyField", JOptionPane.PLAIN_MESSAGE);
					getAccountNo ();
				}
				else {
					findRec (printing);
		...
		try {
			...
			if (total == 0) {
				JOptionPane.showMessageDialog (null, "Records File is Empty.\nEnter Records First to Display.",
					 "Digital Financial Service System - EmptyFile", JOptionPane.PLAIN_MESSAGE);
				b = false;
			}
			else {
				b = true;
	...
	void findRec (String rec) {
		...
		if (found == false) {
			JOptionPane.showMessageDialog (this, "Account No. " + rec + " doesn't Exist.",
					 "Digital Financial Service System - WrongNo", JOptionPane.PLAIN_MESSAGE);
			getAccountNo ();
		}
	...
	String makeRecordPrint (int rec) {
		...
		String data;
		String data0 = "            Digital Financial Service System               \n";	//Page Title.

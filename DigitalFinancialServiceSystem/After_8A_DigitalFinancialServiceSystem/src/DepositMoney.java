...
//	private String records[][] = new String [500][6];
	private FileRecord records[] = new FileRecord[500];
	...
	DepositMoney () {
		...
		cboMonth = DateComboBoxHandle.month();
		cboDay = DateComboBoxHandle.day();
		cboYear = DateComboBoxHandle.year();
	...
	void populateArray () {
		...
		try {
			...
			while (true) {
				records[rows] = new FileRecord();
				records[rows++].read(dis);
//				for (int i = 0; i < 6; i++) {
...
//			if (records[x][0].equals (txtNo.getText())) {
			if(records[x]!=null && records[x].getNo().equals(txtNo.getText())) {
				found = true;
				showRec (x);
				break;
			}
	...
	public void showRec (int intRec) {
		FileRecord c = records[intRec];
		txtNo.setText(c.getNo());
		txtName.setText(c.getName());
		curr = Integer.parseInt(c.getDeposit());
...
//		curr = Integer.parseInt (records[intRec][5]);
	...
	public void editRec () {
		...
		records[recCount] = new FileRecord(
				txtNo.getText (),
				txtName.getText (),
				"" + cboMonth.getSelectedItem (),
				"" + cboDay.getSelectedItem (),
